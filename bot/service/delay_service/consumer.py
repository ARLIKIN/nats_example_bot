...
class DelayedMessageConsumer:
    ...
    async def on_message(self, msg: Msg):
        ...
        sent_time = datetime.fromtimestamp(
            float(msg.headers.get('Tg-Delayed-Msg-Timestamp')),
            tz=timezone.utc)
        ...
        if sent_time + timedelta(seconds=delay) > datetime.now().astimezone():
            ...
            new_delay = (sent_time + timedelta(
                seconds=delay) - datetime.now().astimezone()).total_seconds()
            ...
            message_id = int(msg.headers.get('Tg-Delayed-Msg-ID'))
